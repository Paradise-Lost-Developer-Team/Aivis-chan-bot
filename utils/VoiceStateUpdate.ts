...
export function setupVoiceStateUpdateHandlers(client: Client) {
    ...
    client.on(Events.VoiceStateUpdate, async (oldState: VoiceState, newState: VoiceState) => {
        try {
            ...
            console.error(`[4th Bot] VoiceStateUpdate エラー:`, error);
        } finally {
            // メモリ使用状況をチェック
            monitorMemoryUsage();
        }
                ...
                determineMessageTargetChannel(guild.id).then((persisted) => {
                    if (!persisted) {
                        try {
                                    ...
                                    try {
                                        ...
                                        if (preferred) {
                                            ...
                                            try {
                                                ...
                                                console.log(`[BotJoin:4th] guild=${guild.id} persisted text-channel selected=${(preferred && preferred.id) || preferred}`);
                                            } catch (e) {
                                                console.error('[BotJoin:4th] persist selected text channel error:', e);
                                            }
                                        }
                                    } catch (e) {
                            ...
                            console.error('[BotJoin:4th] 内部処理エラー:', e);
                        }
                    }
                }).catch(e => {
                    console.error('[BotJoin:4th] determineMessageTargetChannel エラー:', e);
                });
            }
        } catch (e) {
            console.error('[BotJoin:4th] エラー:', e);
        }
    });
}
...
async function checkAndLeaveIfEmpty(guildId: string, voiceChannelId: string, guild: any) {
    try {
        ...
        if (humanMembers.size === 0) {
            console.log(`[4th Bot] 全員退出により自動退出: ギルド ${guild.name}`);
            ...
            if (voiceClient) {
                try {
                    ...
                    console.log(`[4th Bot] ボイス接続を切断しました: ギルド ${guild.name}`);
                } catch (error) {
                    console.error(`[4th Bot] ボイス接続切断エラー:`, error);
                }
            }
        }
    } catch (error) {
        console.error(`[4th Bot] 全員退出チェックエラー:`, error);
    }
}