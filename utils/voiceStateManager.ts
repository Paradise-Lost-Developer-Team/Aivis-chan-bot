    ...
    try {
      ...
      if (!finalTextChannelId) {
        if (guild.systemChannelId) {
          ...
          console.log(`[Reconnect:pro] システムチャンネルを使用: ${guild.systemChannelId} (guild: ${guild.name})`);
        } else {
          ...
          if (firstTextChannel) {
            ...
            console.log(`[Reconnect:pro] 最初のテキストチャンネルを使用: ${firstTextChannel.name} (guild: ${guild.name})`);
          } else {
            console.warn(`[Reconnect:pro] 適切なテキストチャンネルが見つからないためスキップ: guildId=${guildId}`);
            skipped++;
            continue;
          }
        }
