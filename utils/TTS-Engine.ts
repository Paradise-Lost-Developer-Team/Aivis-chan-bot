    ...
    class AivisAdapter {
        ...
        constructor() {
            ...
            this.checkServiceHealth()
                ...
                .catch(err => {
                    console.error(`TTSサービス接続確認中にエラーが発生しました: ${err?.message || err}`);
                });
            ...
            if (voiceChannelId && client) {
                ...
                if (guild) {
                    ...
                    if (vc && tc && tc.type === ChannelType.GuildText) {
                        ...
                        // 2) api-mapped channel exists (include voice->text mapping)
                        try {
 ...
 * アナウンス用: 必ずデフォルト話者で再生
 */
function resolveGuildIdFromVoiceOrGuildId_6th(id: string, client?: any): string | undefined {
...
export function speakAnnounce(text: string, voiceOrGuildId: string, client?: any): Promise<void> {
    const guildId = resolveGuildIdFromVoiceOrGuildId_6th(voiceOrGuildId, client) ?? voiceOrGuildId;
    const queue = getQueueForUser(guildId);
