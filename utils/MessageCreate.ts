...
import { voiceClients, loadAutoJoinChannels, MAX_TEXT_LENGTH, speakVoice, speakAnnounce, getTextChannelFromMapByGuild, textChannelByVoice } from './TTS-Engine';
        ...
        if (voiceChannelId) {
            const voiceChannel = message.guild?.channels.cache.get(voiceChannelId) as VoiceChannel;
                ...
                if (voiceChannel) {
                    ...
                    //  2) saved mapping from primary/getTextChannelFromMapByGuild
                    //  3) autoJoin/joinChannels settings
                    //  4) same-name text channel (strict case-insensitive match)

                    // 1) explicit mapping (voiceChannelId -> textChannel)
                    try {
                    ...
                    // 2) same-name text channel only (keep as fallback, with strict permission check)
                    ...
                    if (matchingTextChannel && matchingTextChannel.id === currentChannel.id) {
                        // ensure bot can send messages there
                        try {
