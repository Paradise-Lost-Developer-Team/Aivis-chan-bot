...
exports.ConversationTrackingService = void 0;
...
class ConversationTrackingService {
     ...
     * システム全体の統計を取得
     */
    async getSystemConversationStats() {
        // 全サーバーのメッセージ総数を集計
        let totalMessages = 0;
    ...
    constructor(client) {
        this.userStats = new Map(); // サーバーID -> ユーザーID -> 統計
        this.serverStats = new Map(); // サーバーID -> 統計
        this.saveInterval = null;
        this.voiceTimeTracking = new Map(); // サーバーID -> ユーザーID -> 開始時間
        this.client = client;
    ...
    async trackMessage(message) {
        try {
            ...
            message.mentions.users.forEach(mentionedUser => {
                if (mentionedUser.id === this.client.user.id)
                    return; // Botへのメンションは除外
            ...
            (0, errorLogger_1.logError)('conversationTrackingError', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * ボイスアクティビティの追跡
    ...
    trackVoiceActivity(oldState, newState) {
        try {
            ...
            (0, errorLogger_1.logError)('voiceTrackingError', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * リアクションの追跡
     */
    async trackReactionAdd(reaction, user, details) {
        try {
            if (user.bot)
                return;
            ...
            (0, errorLogger_1.logError)('reactionTrackingError', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * 定期保存の設定
    ...
    saveStats() {
        try {
            ...
            (0, errorLogger_1.logError)('statsDataSaveError', error instanceof Error ? error : new Error(String(error)));
        }
    }
    /**
     * 統計データの読み込み
    ...
    loadStats() {
        try {
            ...
            if (!(error instanceof Error && error.message.includes('no such file or directory'))) {
                (0, errorLogger_1.logError)('statsDataLoadError', error instanceof Error ? error : new Error(String(error)));
            }
        }
    }
    /**
     * 初期ユーザー統計オブジェクトの作成
     */
    createInitialUserStats(userId, username) {
        return {
            userId,
            username,
            totalMessages: 0,
     ...
     * 初期サーバー統計オブジェクトの作成
     */
    createInitialServerStats(serverName, serverId) {
        return {
            serverId,
            serverName,
            totalMessages: 0,
     ...
     * 初期チャンネル統計オブジェクトの作成
     */
    createInitialChannelStats(channelId, channelName) {
        return {
            channelId,
            channelName,
            totalMessages: 0,
...
exports.ConversationTrackingService = ConversationTrackingService;
function updateChannelStat(channelStat, userId, message) {
...
//# sourceMappingURL=conversation-tracking-service.js.map