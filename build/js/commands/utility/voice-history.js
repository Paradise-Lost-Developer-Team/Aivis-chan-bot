...
exports.handleHistoryPagination = handleHistoryPagination;
const builders_1 = require("@discordjs/builders");
const discord_js_1 = require("discord.js");
const subscription_1 = require("../../utils/subscription");
const voiceHistory_1 = require("../../utils/voiceHistory");
module.exports = {
    ...
    async execute(interaction) {
        ...
        // Pro版以上か確認
        if (!(0, subscription_1.isProFeatureAvailable)(guildId, 'voice-history')) {
...
async function handleListCommand(interaction) {
    ...
    const history = (0, voiceHistory_1.getVoiceHistory)(guildId);
    if (history.length === 0) {
        await interaction.reply({
    ...
    let components = [];
    if ((0, subscription_1.isPremiumFeatureAvailable)(guildId, 'voice-history') && history.length > count) {
...
async function handleSearchCommand(interaction) {
    ...
    const searchResults = (0, voiceHistory_1.searchVoiceHistory)(guildId, query);
...
async function handleUserCommand(interaction) {
    ...
    const userHistory = (0, voiceHistory_1.getUserVoiceHistory)(guildId, targetUser.id);
    ...
    const embed = createHistoryEmbed(`${targetUser.username} さんの読み上げ履歴 (${userHistory.length}件中${displayResults.length}件表示)`, displayResults);
    await interaction.reply({
        embeds: [embed],
        flags: discord_js_1.MessageFlags.Ephemeral
    });
...
// 履歴クリアコマンド
async function handleClearCommand(interaction, guildId) {
    // Premium版限定機能
    if (!(0, subscription_1.isPremiumFeatureAvailable)(guildId, 'voice-history')) {
        await interaction.reply({
    ...
    try {
        ...
        if (confirmation.customId === 'history_clear_confirm') {
            const success = (0, voiceHistory_1.clearVoiceHistory)(guildId);
            if (success) {
...
// 履歴表示用のEmbed作成
function createHistoryEmbed(title, history) {
...
async function handleHistoryPagination(interaction, history, currentPage, itemsPerPage) {
    ...
    const row = new discord_js_1.ActionRowBuilder()
        ...
        .setLabel('前のページ')
        .setStyle(discord_js_1.ButtonStyle.Primary)
        .setDisabled(newPage <= 1), new discord_js_1.ButtonBuilder()
        .setCustomId('history_next')
        .setLabel('次のページ')
        .setStyle(discord_js_1.ButtonStyle.Primary)
...
//# sourceMappingURL=voice-history.js.map