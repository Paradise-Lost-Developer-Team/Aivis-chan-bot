...
exports.ReminderCommand = exports.data = void 0;
exports.execute = execute;
const discord_js_1 = require("discord.js");
const voice_reminder_1 = require("../../utils/voice-reminder");
...
async function execute(interaction) {
    // クラス実装に委譲
    await new ReminderCommand(new voice_reminder_1.VoiceReminder()).executeInteraction(interaction);
}
class ReminderCommand {
    constructor(voiceReminder) {
    ...
    getCommandData() {
        return new builders_1.SlashCommandBuilder()
            ...
            .setName('set')
            .setDescription('新しいリマインダーを設定します')
            .addStringOption(option => option.setName('時間')
            .setDescription('リマインダーの時間（例: 30m, 1h30m, 17:30）')
            .setRequired(true))
            .addStringOption(option => option.setName('メッセージ')
            .setDescription('リマインダーのメッセージ')
            .setRequired(true))
            .addBooleanOption(option => option.setName('音声')
    ...
    async executeInteraction(interaction) {
        ...
        if (subcommand === '設定') {
            ...
            try {
                ...
                const response = await this.voiceReminder.setReminder(interaction, // TypeScript型の問題を回避
                message, time, useVoice);
                await interaction.reply({ content: response, ephemeral: false });
            }
            catch (error) {
                await interaction.reply({
                    content: `リマインダーの設定に失敗しました: ${error.message}`,
                    ephemeral: true
                });
            }
        }
    ...
    async executeMessage(message, args) {
        ...
        if (args[0] === '設定') {
            ...
            try {
                const time = this.parseTime(timeStr);
                const response = await this.voiceReminder.setReminder(message, messageText, time, useVoice);
                message.reply(response);
            }
            catch (error) {
                message.reply(`リマインダーの設定に失敗しました: ${error.message}`);
            }
        }
        else if (args[0] === '一覧') {
    ...
    showHelp(message) {
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .setColor('#00BFFF')
            .addFields({ name: '!リマインダー 設定 [時間] [メッセージ]', value: '指定した時間にリマインダーを設定します' }, { name: '!リマインダー 設定 [時間] [メッセージ] --音声', value: '音声付きリマインダーを設定します（有料プラン限定）' }, { name: '!リマインダー 一覧', value: '設定したリマインダーの一覧を表示します' }, { name: '!リマインダー キャンセル [ID]', value: '指定したIDのリマインダーをキャンセルします' }, { name: '時間の指定例', value: '30m (30分後), 1h30m (1時間30分後), 17:30 (17時30分)' });
        message.reply({ embeds: [embed] });
    }
    // 時間文字列をDateオブジェクトに変換
    parseTime(timeStr) {
