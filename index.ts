...
apiApp.post('/internal/join', async (req: Request, res: Response) => {
    try {
        ...
        if (!guildId || !voiceChannelId) {
            return res.status(400).json({ error: 'guildId and voiceChannelId are required' });
        }
        ...
        if (!voiceChannel || !('type' in voiceChannel) || (voiceChannel as any).type !== 2) { // 2 = GuildVoice
            return res.status(400).json({ error: 'voice-channel-invalid' });
        }
        // 既に別VCに接続中ならBusyを返す
        ...
        await new Promise<void>((resolve) => {
            ...
            const cleanup = () => {
                ...
                connection.off(VoiceConnectionStatus.Disconnected, onDisc);
            };
...
apiApp.get('/internal/info', async (req: Request, res: Response) => {
    try {
        ...
        return res.json({
            ...
            guildIds,
            connectedGuildIds,
            vcCount: client.voice.adapters.size,
